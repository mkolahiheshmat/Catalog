//
//  CategoriesLandingPageViewController.m
//  Catalog
//
...
#import "CategoriesLandingPageCustomCell.h"
...
@interface CategoriesLandingPageViewController ()<UIGestureRecognizerDelegate, UITableViewDelegate, UITableViewDataSource>{
...
@implementation CategoriesLandingPageViewController
...
- (void)viewDidLoad {
    ...
    //[[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(fetchPostForCategory:) name:@"setCategoryLanding" object:nil];
    //[[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(fetchPostForCategory2:) name:@"setCategory2" object:nil];
    //[[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(refreshTable:) name:@"FetchTagDescription" object:self.dictionary];
    ...
    [self fetchNEWPostWithCategory:[self.dictionary objectForKey:@"id"] WithRequest:@"" WithDate:@""];
}
...
-(void) makeTopBar{
    ...
    titleLabel.text = [self.dictionary objectForKey:@"name"];
    ...
    UIButton *categoryButton = [[UIButton alloc]initWithFrame:CGRectMake(18, 20, 64 * 0.5, 64 * 0.5)];    [categoryButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal ];
...
- (void)populateTableViewArray{
    ...
    NSArray *array = [Database selectFromCategoriesLandingPageWithFilePath:[Database getDbFilePath]];
    [noDataLabel removeFromSuperview];
    if ([array count] == 0) {
        noDataLabel = [[UILabel alloc]initWithFrame:CGRectMake(screenWidth-220,21 , 200, 25)];
        noDataLabel.text = @"مطلبی وجود ندارد";
        NSString *customColor = [[NSUserDefaults standardUserDefaults]objectForKey:@"customColor"];
        if ([customColor length]>0) {
            UIColor *color = [ConvertHexToColor colorWithHexString:customColor];
            noDataLabel.textColor = color;
        } else {
            noDataLabel.textColor = MAIN_COLOR;
        }
        noDataLabel.numberOfLines = 1;
        noDataLabel.font = FONT_MEDIUM(13);
        noDataLabel.baselineAdjustment = YES;
        noDataLabel.adjustsFontSizeToFitWidth = YES;
        noDataLabel.minimumScaleFactor = 0.5;
        noDataLabel.clipsToBounds = YES;
        noDataLabel.backgroundColor = [UIColor clearColor];
        noDataLabel.textAlignment = NSTextAlignmentRight;
...
- (void)fetchPostForCategory:(NSNotification *)notif{
    ...
    NSArray *array = [Database selectFromLandingPageWithTagName:_categoryId filePath:[Database getDbFilePath]];
...
- (void)savePostImageIntoDocumetsWithData:(NSData*)imageData2 withName:(NSString*)imageName2{
    
    NSString *tempName2 = [imageName2 lastPathComponent];
    ...
    //create the file
    [[NSFileManager defaultManager] createFileAtPath:imgFileName2 contents:imageData2 attributes:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CategoriesLandingPageCustomCell *cell = (CategoriesLandingPageCustomCell *)[[CategoriesLandingPageCustomCell alloc]
...
- (void)fetchNEWPostWithCategory:(NSString *)categoryId WithRequest:(NSString *)request WithDate:(NSString *)date{
    ...
    if ([self.dictionary valueForKey:@"icon"] != nil) {
        params = @{
                   @"category_id":[self.dictionary objectForKey:@"id"],
                   @"tag_id":@"",
        ...
        params = @{
                   @"category_id":@"",
                   @"tag_id":[self.dictionary objectForKey:@"id"],
                   @"author_id":CUSTOMER_AUTHOR_ID,
                   @"date":date
                   };
    }
    /*http://catalog2.yarima.co/mobiles/news2*/
    NSString *url = [NSString stringWithFormat:@"%@news2", BaseURL3];
    ...
    if (!isBusyNow) {
        ...
        [manager POST:url parameters:params progress:nil success:^(NSURLSessionTask *operation, id responseObject) {
            [ProgressHUD dismiss];
            [Database deleteCategoriesLandingPageWithFilePath:[Database getDbFilePath]];
